...
// 积分
const winIntegral = reactive(new Map([['black', 0], ['white', 0]]))
// 标题
...
const blackCount = computed(() => {
    return winIntegral.get(opponentBlack)
})
// 白方积分总和
const whiteCount = computed(() => {
    return winIntegral.get(opponentWhite)
})
// 此刻落子方是否是黑方
...
// 黑方积分加1
const blackAddOne = () => {
    let num = winIntegral.get(opponentBlack)
    num++
    winIntegral.set(opponentBlack, num)
}
// 白方积分加1
const whiteAddOne = () => {
    let num = winIntegral.get(opponentWhite)
    num++
    winIntegral.set(opponentBlack, num)
}
// 格子个数
